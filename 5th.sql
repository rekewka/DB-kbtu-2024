...
select count(customers.cust_name) from customers

select min(orders.purch_amt) from orders

select * from customers where cust_name like '%b'

select * from orders left join customers c on c.customer_id = orders.customer_id
where c.city = 'New York'

select * from customers join orders on customers.customer_id = orders.customer_id
where orders.purch_amt > 10

select count(customers.grade) from customers
